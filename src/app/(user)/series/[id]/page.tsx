    ...
    <div>
      ...
      <div className="flex flex-col md:flex-row gap-4 md:gap-6 mb-8">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm text-muted mt-2">{series.contents?.length || 0} contents</p>
        </div>
      </div>

      <h2 className="text-lg font-medium text-foreground mb-4">Contents</h2>
      <div className="space-y-3">
        {(series.contents || []).map((c: any) => (
          <div
            key={c.id}
            onClick={() => router.push(`/content/${c.id}`)}
            className="bg-surface border border-border rounded-xl p-3 md:p-4 flex items-center gap-3 md:gap-4 cursor-pointer hover:border-accent/50 transition-colors group"
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                {c.orderInSeries && <span className="text-xs text-accent font-medium">#{c.orderInSeries}</span>}
                <h3 className="text-sm font-medium text-foreground truncate group-hover:text-accent transition-colors">{c.title}</h3>
              </div>
              <div className="flex items-center gap-3 text-xs text-muted mt-1">
                <span>{c.totalItems} items</span>
                <span>{c.isFree ? 'Free' : `${c.tokenPrice} tokens`}</span>
                <span className="hidden sm:inline">{new Date(c.createdAt).toLocaleDateString()}</span>
              </div>
            </div>
            {!c.isUnlocked && !c.isFree && (
        ...
        {(!series.contents || series.contents.length === 0) && (
          <p className="text-sm text-muted py-8 text-center">No contents yet.</p>
        )}
      </div>
    </div>
  );
